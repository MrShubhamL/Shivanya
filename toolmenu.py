...
class Sales(QMainWindow,ui):
    def __init__(self):
        ...
        self.lineEdit_23.setText("Shubham")
    ...
    def add_QR_data(self):
        ...
        self.db = sqlite3.connect('DB\db.db')
        self.cur = self.db.cursor()
        Device_Name = self.lineEdit_24.text();
        sql = ''' SELECT * FROM cameraSetting WHERE DeviceName = ? '''
        self.cur.execute(sql , [(Device_Name)])
...
class Purchase(QMainWindow,pi):
    ...
    def add_products(self):
        ...
        if Purchase_Bill == "":
            ...
            try:
                data = (Purchase_Bill,Supp_Bill,Barcode,Product_Name,Artical_No,MRP,Sale_Rate,Rate,Net_Amount,Quantity,Packing,Storage,Date,Discont,GST,
...
class Serial_Number(QMainWindow,serial):
    ...
    def buttons(self):
        ...
        self.pushButton_4.clicked.connect(self.deleteMAC)
    ...
    def get_MAC(self):
        c = wmi.WMI()
        logical_disk = c.Win32_LogicalDisk(Caption='C:')[0]
        partition = logical_disk.associators()[1]
        physical_disc = partition.associators()[0]
        print(physical_disc.SerialNumber) 

        self.lineEdit.setText(str(physical_disc.SerialNumber))
    

    def Add_MAC(self):
        # self.db = mysql.connector.connect(host='localhost' , user='root' , password ='shubh96S@' , db='data')
        MACNumber = self.lineEdit_2.text()
        MACAddress = self.lineEdit.text()


        if MACNumber == "" or MACAddress == "" :
            ...
            self.db.commit()
            self.db.close()
            message3 = QMessageBox.information(self,"Successful!","Details Added Successfully")
...
class CameraSetting(QMainWindow,bi):
    ...
    def handlebutton(self):
        self.pushButton_9.clicked.connect(self.addSetting)
        pass

    def addSetting(self):

        Device_Name = self.lineEdit_15.text();
        Device = self.comboBox_3.currentText();
        Frame1 = self.lineEdit_16.text();
        Frame2 = self.lineEdit_17.text();
        # try:
        self.db = sqlite3.connect('DB\db.db')
        self.cur = self.db.cursor()
        data = (Device_Name,Device,Frame1,Frame2)
        sql = ''' INSERT INTO cameraSetting VALUES (?,?,?,?) '''
        self.cur.execute(sql,data)
        self.db.commit()
        message = QMessageBox.information(self,"Successfull","Setting added successfully")
        self.db.close()
        # except:
        #     message = QMessageBox.information(self,"Error","Internal Server Error")

        pass

li,_ = loadUiType('ToolBar\WindowSetting.ui')
class WindowSetting(QMainWindow,li):
    def __init__(self):
        super().__init__()
        self.setGeometry(150,100,469,341)
        # self.setWindowIcon(QIcon,("salon.jpg"))
        self.setupUi(self)
        self.handlebutton()
        # self.actions()
    ...
    def handlebutton(self):
        self.pushButton_9.clicked.connect(self.SetWindow)
        self.pushButton_2.clicked.connect(self.UpdateWindow)

    def UpdateWindow(self):
        X_Axis = self.lineEdit_18.text()
        Y_Axis = self.lineEdit_19.text()
        Resolution1 = self.lineEdit_16.text()
        Resolution2 = self.lineEdit_17.text()



        self.db = sqlite3.connect('DB\db.db')
        self.cur = self.db.cursor()
        self.cur.execute(f"UPDATE windowSetting SET x = {X_Axis}, y = {Y_Axis}, resolution1 = {Resolution1}, resolution2 = {Resolution2}")
        self.db.commit()
        self.db.close()
        messesage = QMessageBox.information(self,"Update","Successfull")
    ...
    def SetWindow(self):
        ...
        data9 = (Id,X_Axis,Y_Axis,Resolution1,Resolution2)
        sql = ''' INSERT INTO windowSetting VALUES (?,?,?,?,?) '''
        self.cur.execute(sql,data9)
        self.db.commit()
        message2 = QMessageBox.information(self,"Successfull!","Resolution Save Successfully!.")
        self.db.close()
